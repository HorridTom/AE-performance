...
library(tidyr)
library(scales)
knitr::opts_chunk$set(echo = FALSE, warning = FALSE, message = FALSE, fig.path = 'perf_images/')
...
```{r combined-trusts, echo=FALSE, eval=TRUE, include=FALSE}
pr<-providerCodes
pr_name <- "Combined Trusts"
  ...
  env=new.env() #create a new empty environment, it inherits objects from the current         environment.
    ...
    cat("\\newpage")
```
```{r individual-orgs, echo=FALSE, eval=TRUE, include=FALSE}
all_provs <- FALSE
